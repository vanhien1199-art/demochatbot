...
export const generateGameContent = async (config: GameConfig): Promise<GeneratedContent> => {
  ...
  1. Hãy TÌM KIẾM (dùng công cụ) nội dung thực tế của bài học này trong SGK (ví dụ, tình huống mở đầu, hình ảnh, bài tập).
  ...
  *** LOẠI 5: wheel (Vòng quay) ***
  ...
  try {
    ...
    throw new Error("Lỗi kết nối Gemini. Vui lòng thử lại sau giây lát.");
  }
};
